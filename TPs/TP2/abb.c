...
bool apilar_siguiente_inorder(abb_iter_rango_t* iter, abb_nodo_t* nodo){
	...
	while(actual){
		...
		if(iter->cmp(actual->clave,iter->desde)<0) apilar_siguiente_inorder(iter,actual->der);

		if(iter->cmp(actual->clave,iter->desde)>=0 && iter->cmp(actual->clave, iter->hasta)<=0) pila_apilar(iter->pila, actual);
...
abb_iter_rango_t *abb_iter_rango_crear(const abb_t *arbol, char* desde, char* hasta){
	...
	if(!apilar_siguiente_inorder(iter,arbol->raiz)) return NULL; 
...
bool abb_iter_rango_avanzar(abb_iter_rango_t *iter){
	...
	if(!apilar_siguiente_inorder(iter,act->der)) return false;
