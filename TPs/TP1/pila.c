...
bool redimensionar(pila_t *pila, int porcentaje){
...
bool pila_apilar(pila_t *pila, void* valor){
	if (pila->capacidad==pila->cantidad){
		if (!redimensionar(pila, PORCENTAJE_AUMENTO_MEMORIA)) return false;
	}
...
void* pila_desapilar(pila_t *pila){
	...
	if ( pila->cantidad < (pila->capacidad*PORCENTAJE_DISMINUCION_MEMORIA)/100 && pila->capacidad > (TAMANIO_INICIAL*(100+PORCENTAJE_DISMINUCION_MEMORIA))/100){
		redimensionar(pila,PORCENTAJE_DISMINUCION_MEMORIA);
	}
	return pila->datos[pila->cantidad];
}

size_t pila_len( pila_t* pila){
