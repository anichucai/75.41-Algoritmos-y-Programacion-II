...
void prueba_iterar_vacio(){

    printf("%s\n", "PRUEBAS ITERAR VACIO");

    abb_t* abb = abb_crear(strcmp,NULL);
    abb_iter_t* iter = abb_iter_in_crear(abb);
    print_test("Prueba iter crear iterador abb vacio", iter);
    ...
    print_test("Prueba iter ver actual es NULL", !abb_iter_in_ver_actual(iter));
...
bool corte_rama_izquierda(const char* clave,void* dato,void* extra){
...
void pruebas_iter_interno(){
    ...
    char* claves[5] = {"4", "5", "3","1","2"};
    int valores[5] = {4, 5, 3, 1, 2};
    int suma = 0;

    abb_in_order(abb, sumar_todos, &suma);
    print_test("Se trato de iterar internamente una abb vacio", true);

    for( int i=0;i<5;i++ ) abb_guardar(abb, claves[i], &valores[i]);
    
    abb_in_order(abb, sumar_todos, &suma);
    print_test("Se itero internamente un abb utilizando el parametro extra.",suma==15);

    suma=0;

    abb_in_order(abb, corte_rama_izquierda, &suma);
    print_test("Se itero internamente un abb con utilizando una condicion de corte.",suma==3);

    abb_destruir(abb);
}

void pruebas_iter_rango(){

    printf("%s\n", "PRUEBAS ITER EXTERNO");
    
    abb_t* abb = abb_crear(strcmp,NULL);

    char *claves[] = {"perro", "gato", "vaca", "elefante", "gorilla"};
    char *valores[] = {"A", "B", "C", "D", "E"};

    /* Inserta 3 valores */
    print_test("Prueba abb insertar 5 claves", abb_guardar(abb, claves[0], valores[0]) && abb_guardar(abb, claves[1], valores[1]) && abb_guardar(abb, claves[2], valores[2]) && abb_guardar(abb, claves[3], valores[3]) && abb_guardar(abb, claves[4], valores[4]));

    // Prueba de iteración sobre las claves almacenadas.
    abb_iter_rango_t* iter = abb_iter_rango_crear(abb, "B", "C", (cmp_func_t)strcmp);

    print_test("Prueba abb iterador esta al final, es false", !abb_iter_rango_al_final(iter));


    print_test("Prueba abb iterador B", strcmp((char*)abb_iter_rango_ver_actual(iter),valores[1]));
    print_test("Prueba abb iterador avanzar es true", abb_iter_rango_avanzar(iter));
    print_test("Prueba abb iterador C", strcmp((char*)abb_iter_rango_ver_actual(iter),valores[2]));
    print_test("Prueba abb iterador avanzar es true", abb_iter_rango_avanzar(iter));
    print_test("Prueba abb iterador D", strcmp((char*)abb_iter_rango_ver_actual(iter),valores[3]));
    print_test("Prueba abb iterador avanzar es true", abb_iter_rango_avanzar(iter));
